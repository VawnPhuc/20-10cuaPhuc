window.addEventListener('load', () => {
    ...
    function createFallingImage() {
        ...
        // Khi click vào ảnh, đổi sang ảnh mình muốn
        el.addEventListener('click', () => {
            const bigImg = document.createElement('img');
            bigImg.src = `style/img2/Me (${Math.floor(Math.random() * 5) + 1}).png`;
            bigImg.className = 'popup-img';
            document.body.appendChild(bigImg);

            // Tạo một lớp nền mở để click ra ngoài
            const overlay = document.createElement('div');
            overlay.className = 'popup-overlay';
            document.body.appendChild(overlay);

            // Đưa ảnh lên trên overlay
            overlay.appendChild(bigImg);

            // Khi click vào ảnh lớn, đóng lại
